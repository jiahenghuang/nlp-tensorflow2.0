...
from corpus_reader import CGEDReader, save_word_dict, load_word_dict
from seq2seq_model import create_model, callback, evaluate
...
def train(train_path=None,
    ...
    if os.path.exists(save_model_path) and os.path.exists(encoder_model_path):
        ...
        model, encoder_model, decoder_model = create_model(len(input_token_index), len(target_token_index),
                                                           rnn_hidden_dim)
    # Run training
